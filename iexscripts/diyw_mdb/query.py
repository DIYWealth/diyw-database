...
def get_active_companies():
    ...
    symbols = get_symbols()
    companies = get_company( symbols["symbol"].tolist() )
...
def get_chart(ref_symbol, ref_date = "1990-01-01", when = "after"):
    """
...
def get_quotes(ref_symbol, ref_date = "1990-01-01", when = "after"):
    """
    ...
    if when == "after" or when == "on":
        results = db.iex_quotes.find( query ).sort("date", DESCENDING)
    else:
        results = db.iex_quotes.aggregate( query )
        #results = db.iex_quotes.find( query )
...
def get_dividends(ref_symbol, ref_date = "1900-01-01", when = "after"):
    """
...
def get_earnings(ref_symbol, ref_date = "1900-01-01", when = "after", date_type = "fiscalEndDate"):
    """
...
def get_financials(ref_symbol, ref_date = "1900-01-01", when = "after"):
    """
...
def get_balancesheets(ref_symbol, ref_date = "1900-01-01", when = "after"):
    """
    ...
    if when == "after":
        ...
        results = db.iex_balancesheets.aggregate( query )
...
def get_stats(ref_symbol, ref_date = "1900-01-01", when = "after"):
    """
    Return company balance sheet from MongoDB
    @params:
        ref_symbol  - Required  : symbol list ([Str])
    ...
    if when == "after":
        results = db.iex_stats.find( query ).sort("date", DESCENDING)
    else:
        results = db.iex_stats.aggregate( query )
...
def get_transactions(portfolioID, date, when = "on"):
    """
...
def get_holdings(portfolioID, date = " 1990-01-01", when = "on"):
    """
...
def get_performance(ref_portfolioID, ref_date = "1990-01-01"):
    """
...
def get_stock_list(ref_date = "1990-01-01", when = "on"):
    """
