...
import numpy
import pandas
from iexscripts.iex import reference
from iexscripts.iex import Stock
...
def get_symbols(ref_symbol=None, ref_type=None):
    """
...
def get_company(ref_symbol):
    ...
    if not company.empty:
        company = set_column_sequence(company, ["symbol","companyName","exchange","industry","description","securityName","issueType","sector"])
        #print( company )

    return company

def get_chart(ref_symbol, ref_range='1m'):
    """
    Get charts from IEX
    @params:
...
def get_quote(ref_symbol):
    ...
    if not quote.empty:
        ...
        quote["date"] = date
        #Reorder dataframe
        quote = set_column_sequence(quote, ["symbol","date","close","marketCap","peRatio"])

    return quote

def get_dividends(ref_symbol, ref_range='1m'):
    """
...
def get_balancesheets(ref_symbol):
    """
    Get financials from IEX
    @params:
        ref_symbol  - Required  : symbol (Str)
    ...
    if not balancesheet.empty:
        ...
        #Reorder dataframe
        balancesheet = set_column_sequence(balancesheet, ["symbol","reportDate"])

    return balancesheet

def get_stats(ref_symbol):
    """
    Get stats from IEX
    @params:
        ref_symbol  - Required  : symbol (Str)
    ...
    if not stats.empty:
        ...
        #Reorder dataframe
        stats = set_column_sequence(stats, ["symbol","date","sharesOutstanding"])
